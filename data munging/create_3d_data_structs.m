...
% %             clear lfp_data_mv
%               bettyGoodTrials.(day).(trial) = lfp_data_mv(:,trial_info.CueOnset(j)-500:end); %store shifted time series with 500ms prestimulus baseline as start
%               offset = c - size(bettyGoodTrials.(day).(trial),2); %calc offset for revised markers
...
%               new_trial_info.BehResp(z) = trial_info.BehResp(j);
%               new_trial_info.Rule(z) = trial_info.rule(j);
%               z = z + 1;
...
%         end
%     end
%     bettyGoodTrials.(day).new_trial_info = new_trial_info;
    bettyGoodTrials.(day).recording_info = recording_info;
...
% %                 end
% %                 end
%               clarkGoodTrials.(day).(sesh).(trial) = lfp_data_mv(:,floor(trial_info.CueOnset(k)-500):end); %store shifted time series with 500ms prestimulus baseline as start
%               offset = c - size(clarkGoodTrials.(day).(sesh).(trial),2); %calc offset for revised markers
...
%               new_trial_info.BehResp(z) = trial_info.BehResp(k);
%               new_trial_info.Rule(z) = trial_info.rule(k);
%               z = z + 1;
%             end
%         end
%         clarkGoodTrials.(day).(sesh).new_trial_info = new_trial_info;
        clarkGoodTrials.(day).(sesh).recording_info = recording_info;
%         day = sprintf('d%s',days_clark(i));
